...
public class ArticleListActivity extends AbstractNewsRobListActivity implements IEntryModelUpdateListener,
    ...
    protected static final int MENU_ITEM_MARK_READ_UNTIL_HERE_ID = 112;
    ...
    private class MyListAdapter extends ResourceCursorAdapter {
        ...
        void bindView(final View view, final Context context, final Cursor cursor, final EntryManager entryManager) {
            ...
            final boolean isShared = DB.getBooleanValueFromCursor(cursor, columnIndices.sharedStateIndex);
            final boolean isStarred = DB.getBooleanValueFromCursor(cursor, columnIndices.starredStateIndex);
            final boolean isLiked = DB.getBooleanValueFromCursor(cursor, columnIndices.likedStateIndex);
            final boolean isAnnotated = DB.getStringValueFromCursor(cursor, columnIndices.noteIndex) != null;
            ...
            holder.feedTitleTextView.setCompoundDrawablesWithIntrinsicBounds(getResources().getDrawable(
                    uiHelper.getArticleDownloadIndicatorDrawable(downloaded, downloadPref, getResources())), null,
                    null, null);
            ...
            if (NewsRob.SHOW_CHANGED) {
                final boolean isStatePending = DB
                        ...
                        || DB.getBooleanValueFromCursor(cursor, columnIndices.sharedStatePendingIndex)
                        || DB.getBooleanValueFromCursor(cursor, columnIndices.starredStatePendingIndex)
                        || DB.getBooleanValueFromCursor(cursor, columnIndices.likedStatePendingIndex) || isAnnotated;

                holder.articleChangedTextView.setVisibility(isStatePending ? View.VISIBLE : View.GONE);
            }

            holder.articleSharedTextView.setVisibility(isShared ? View.VISIBLE : View.GONE);
            holder.articleStarredTextView.setVisibility(isStarred ? View.VISIBLE : View.GONE);
            // holder.articleLikedTextView.setVisibility(isLiked ? View.VISIBLE
            // : View.GONE);
            holder.articleAnnotatedTextView.setVisibility(isAnnotated ? View.VISIBLE : View.GONE);
            ...
            final String sharedByFriend = DB.getStringValueFromCursor(cursor, columnIndices.sharedByFriendIndex);
            if (sharedByFriend != null) {
                holder.sharedByFriendTextView.setVisibility(View.VISIBLE);
                holder.sharedByFriendTextView.setText("Shared by " + sharedByFriend);
            } else
                holder.sharedByFriendTextView.setVisibility(View.GONE);
    ...
    static class DBColumnIndices {
        ...
        int sharedStateIndex;
        int downloadedIndex;
        int downloadPrefIndex;
        int readStatePendingIndex;
        int sharedStatePendingIndex;
        int starredStatePendingIndex;
        int likedStatePendingIndex;
        int sharedByFriendIndex;
        int likedStateIndex;
        int noteIndex;
        int atomIdIndex;
        int snippetIndex;
        ...
        DBColumnIndices(final Cursor c) {
            ...
            sharedStateIndex = c.getColumnIndex(DB.Entries.SHARED_STATE);
            likedStateIndex = c.getColumnIndex(DB.Entries.LIKED_STATE);
            ...
            sharedStatePendingIndex = c.getColumnIndex(DB.Entries.SHARED_STATE_PENDING);
            likedStatePendingIndex = c.getColumnIndex(DB.Entries.LIKED_STATE_PENDING);
            starredStatePendingIndex = c.getColumnIndex(DB.Entries.STARRED_STATE_PENDING);
            sharedByFriendIndex = c.getColumnIndex(DB.Entries.SHARED_BY_FRIEND);
            noteIndex = c.getColumnIndex(DB.Entries.NOTE);

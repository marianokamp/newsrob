...
/**
 * The SyncService is based on an app architecture that tries to place network
 * calls (especially larger calls or those called regularly) on independent
 * services, making the activity / fragment a passive receiver of its updates.
 * This, along with data fragments for handling UI updates, throttles network
 * access and ensures the UI is passively updated and decoupled from network
 * calls. Examples of other apps using this architecture include the
 * NBCSportsTalk and Google I/O 2011 apps.
 */
        ...
        try {
            ...
            case EXTRA_TASK_FEED_UPDATE:
                if (!TextUtils.isEmpty(intent.getStringExtra(EXTRA_TASK_FEED_ID))) {
                    StoriesResponse storiesForFeed = apiManager.getStoriesForFeed(
                            intent.getStringExtra(EXTRA_TASK_FEED_ID), intent.getStringExtra(EXTRA_TASK_PAGE_NUMBER),
                            (StoryOrder) intent.getSerializableExtra(EXTRA_TASK_ORDER),
            ...
            case EXTRA_TASK_MULTIFEED_UPDATE:
                if (intent.getStringArrayExtra(EXTRA_TASK_MULTIFEED_IDS) != null) {
                    StoriesResponse storiesForFeeds = apiManager.getStoriesForFeeds(
                            intent.getStringArrayExtra(EXTRA_TASK_MULTIFEED_IDS),
            ...
            case EXTRA_TASK_MULTISOCIALFEED_UPDATE:
                if (intent.getStringArrayExtra(EXTRA_TASK_MULTIFEED_IDS) != null) {
                    SocialFeedResponse sharedStoriesForFeeds = apiManager.getSharedStoriesForFeeds(
                            intent.getStringArrayExtra(EXTRA_TASK_MULTIFEED_IDS),
                            intent.getStringExtra(EXTRA_TASK_PAGE_NUMBER));
            ...
            case EXTRA_TASK_STARRED_STORIES_UPDATE:
                StoriesResponse starredStories = apiManager.getStarredStories(intent
                        .getStringExtra(EXTRA_TASK_PAGE_NUMBER));
            ...
            case EXTRA_TASK_SOCIALFEED_UPDATE:
                if (!TextUtils.isEmpty(intent.getStringExtra(EXTRA_TASK_SOCIALFEED_ID))
                    ...
                    SocialFeedResponse storiesForSocialFeed = apiManager.getStoriesForSocialFeed(
                            intent.getStringExtra(EXTRA_TASK_SOCIALFEED_ID),
                            intent.getStringExtra(EXTRA_TASK_SOCIALFEED_USERNAME),
                            intent.getStringExtra(EXTRA_TASK_PAGE_NUMBER));
            ...
            e.printStackTrace();
            Log.e(this.getClass().getName(), "Couldn't synchronise with Newsblur servers: " + e.getMessage(),
                    e.getCause());
            if (receiver != null) {
